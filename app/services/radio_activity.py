...
def record_traffic_device_station_observation(
    *,
    ...
    # TOCALL is only a statistics label. Do not use it as a gate for device detection.
    # Some APRS packets, especially Mic-E / compressed / non-APxxxx destinations, may still
    # be identifiable by lookup_aprs_device_identification(destination, info).
...
def _build_device_pair_observations_from_frame_rows(
    *,
    ...
    for row in frame_rows:
        ...
        # TOCALL is a display/statistics field only. It must not block device identification.
        # For Mic-E / compressed / non-standard packets the destination may not look like APxxxx,
        # but the device database lookup can still resolve the model using destination + info.
