...
Zakładka TNC konfiguruje interfejsy radiowe używane przez APRSBox do odbioru ramek KISS/TNC2, wysyłki ramek outbound oraz opcjonalnego udostępniania portu KISS w sieci LAN.
...
Wyłączenie TNC zatrzymuje jego użycie w monitorze ruchu i w wysyłce. Konfiguracje beaconu, WX, obiektów, biuletynów i wiadomości mogą nadal wskazywać ten interfejs, ale wysyłka zostanie pominięta albo zakończy się błędem zależnie od kontekstu.

## Typy interfejsu

- `TCP` łączy się z TNC lub programem wystawiającym KISS przez TCP. `Path / Adress` ma zwykle format `host:port`, na przykład `127.0.0.1:8001`.
...
`Baud Rate` jest używany tylko dla `SERIALL`. Dla `TCP` i `OpenWebRX MQTT` jest ignorowany.
...
## Kiedy używać kilku TNC

Kilka aktywnych TNC może działać równolegle. Wtedy ruch odbierany jest osobno per interfejs, a wysyłka zależy od wyboru w danej zakładce, na przykład `My Station`, `WX`, obiektach, biuletynach, wiadomościach albo regułach `Packet Routing`.
