...
`APRS-IS source -> APRS-IS Input Guard -> default-deny callsign + radius filter -> RF TX Guard -> TX RF`

Both guards are inserted automatically for `APRS-IS -> RF`. They cannot be removed, disabled, bypassed, reordered, or added twice. The backend and runtime enforce the same protection even if stored data is modified manually.
