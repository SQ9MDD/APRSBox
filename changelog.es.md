...
- `Docker / multiarch`: `uvloop` y `httptools` siguen activados para las imágenes Linux `amd64` y `arm64`, pero son opcionales en el antiguo `arm/v7`; allí Uvicorn usa `asyncio` y `h11` estándar sin compilar esos paquetes desde código fuente. También se actualizaron las acciones de QEMU y Docker Build & Push.
...
- `Mapa / agrupación de estaciones`: la agrupación de símbolos superpuestos en iconos azules con contador es ahora opcional; un nuevo interruptor global de Ajustes está desactivado de forma predeterminada, por lo que el mapa normalmente muestra iconos APRS individuales.
...
- `Ajustes / ergonomía`: las acciones, la edición de fuentes de mapa y las recargas posteriores al guardado conservan el panel activo y la posición de desplazamiento en vez de volver al inicio de la página.
...
- `Packet Routing / Interfaces / GUI`: se simplificaron los formularios eliminando descripciones y etiquetas repetidas ya cubiertas por la ayuda completa; se mantienen los mensajes de seguridad, la validación y los valores dinámicos de configuración.
...
- `Dashboard`: se rediseñó la pantalla principal con KPI más claros según el rango seleccionado, un gráfico de actividad RF y resúmenes compactos de configuración, servicios y runtime.
- `Condiciones de banda / mapa`: el historial de propagación ofrece rangos de `24h / 7d / 30d / 365d` y un punto para la hora actual; también se corrigieron la última vista del mapa y el ajuste global del relleno de cobertura.
