...
- `Dashboard / configuration help`: replaced the readiness card's configuration link with the standard help icon and added an approachable four-language guide through `Interfaces → My Station → Packet Routing`, including the difference between the station's own `Local TX → APRS-IS` flow and the uplink of frames received over RF.
